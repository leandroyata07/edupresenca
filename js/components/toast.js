...
class AppToast extends HTMLElement {
    ...
    show({ type = 'info', title, message, duration = 4000 }) {
        this._saveNotification({ type, title, message });
    ...
    _saveNotification({ type, title, message }) {
        try {
            ...
            window.dispatchEvent(new CustomEvent('edu-notification-added'));
        } catch (e) {
            console.error('Erro ao salvar notificação no localStorage:', e);
        }
    }
