...
// =========================================================
import { alunos, turmas, cursos, disciplinas, unidades, turnos, presencas, notas, auth } from '../store.js';
...
export function render(outlet) {
    ...
    let allDisciplinas = disciplinas.getAll();
    let allUnidades = unidades.getAll();
    let allTurnos = turnos.getAll();
    ...
    if (isProfessor) {
        ...
        allDisciplinas = allDisciplinas.filter(d => minhasDisciplinas.includes(d.id));
    }
    ...
    // Teachers should never see "Primeiros Passos"
    const showGettingStarted = user?.role !== 'professor';
    ...
    allTurmas.forEach(t => {
        if (isProfessor) {
            ...
            minhasDisciplinas.forEach(discId => {
                const registros = presencas.getByTurmaData(t.id, todayStr, discId);
    ...
    const rankingTurmas = allTurmas.map(t => {
        let total = 0, pres = 0, totalAulas = 0;
        if (isProfessor) {
            const minhasDisciplinas = me.disciplinas || [];
            minhasDisciplinas.forEach(discId => {
                const datas = presencas.getDates(t.id, discId);
                totalAulas += datas.length;
                datas.forEach(d => {
                    const regs = presencas.getByTurmaData(t.id, d, discId);
                    total += regs.length;
        ...
        return { id: t.id, nome: t.nome, cursoNome: curso?.nome || '—', freq, totalAulas };
        ...
        if (d.getDay() !== 0 && d.getDay() !== 6) {
            ...
            allTurmas.forEach(turma => {
                if (isProfessor) {
                    const minhasDisciplinas = me.disciplinas || [];
                    minhasDisciplinas.forEach(discId => {
                        const regs = presencas.getByTurmaData(turma.id, dateStr, discId);
    ...
    <div class="stagger-children">
      ...
      <div class="page-header">
        <div class="page-header-left">
          <h1 class="page-title">
            Dashboard
            <span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:4px;background:rgba(245,158,11,0.18);color:#fbbf24;border:1px solid rgba(245,158,11,0.3);vertical-align:middle;margin-left:8px">1.0.2</span>
          </h1>
          <p class="page-subtitle">Visão geral do sistema — ${formatDate(todayStr)}</p>
        </div>
      </div>

      ${isProfessor && ((me.disciplinas || []).length === 0 || (me.cursos || []).length === 0) ? `
      <!-- Alerta de Atribuição Pendente -->
      <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); border-radius: 12px; padding: 20px; margin-bottom: 24px; display: flex; align-items: flex-start; gap: 16px; box-shadow: 0 4px 12px rgba(239, 68, 68, 0.05);">
        <div style="background: rgba(239, 68, 68, 0.15); color: #ef4444; border-radius: 10px; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
        ...
        <div style="flex: 1;">
          <h3 style="margin: 0 0 6px 0; font-size: 15px; font-weight: 700; color: var(--text-primary);">Atribuições Acadêmicas Pendentes</h3>
          <p style="margin: 0; font-size: 13px; color: var(--text-secondary); line-height: 1.5;">
            Nenhum curso ou disciplina está atrelado ao seu perfil no momento. 
            Isso ocorre se as atribuições no painel administrativo foram vinculadas a um e-mail diferente do que você usou para fazer login (por exemplo, <strong>${escapeHtml(me.email)}</strong>). 
            Solicite a um <strong>Administrador</strong> para verificar seu cadastro no menu <strong>Professores</strong> ou certifique-se de que fez o login com o e-mail exato registrado.
          </p>
        </div>
      </div>
      ` : ''}
      ...
      <div class="content-grid">
              ...
              ${chartDays.map((d, i) => `
                <div class="bar-chart-col">
                  <div class="bar-chart-bar${i % 3 === 1 ? ' second' : i % 3 === 2 ? ' third' : ''}"
                       style="height: ${d.hasData ? Math.max((d.pct / maxPct) * 100, 4) : 0}%"
                       title="${d.label}: ${d.hasData ? d.pct + '%' : 'Sem chamada'}"></div>
            ...
            <div style="margin-top:12px; display:flex; gap:12px; flex-wrap:wrap;">
              ${chartDays.map(d => `
                <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text-secondary)">
                  <strong style="color:${d.hasData ? 'var(--text-primary)' : 'var(--text-tertiary)'}">
                    ${d.hasData ? d.pct + '%' : '—'}
                  </strong>
                  <span>${d.label}</span>
                </div>
        ...
        const freq = hasData ? Math.round((stats.presentes / stats.total) * 100) : null;
        const freqDisplay = hasData ? `${freq}%` : '—';
        const freqColor = hasData ? (freq >= 75 ? 'var(--secondary-400)' : 'var(--danger-400)') : 'var(--text-tertiary)';
        return `
                <div class="detail-row" style="padding:14px 20px; cursor:pointer;" onclick="app.router.push('/turmas')">
                  ...
                  <div style="text-align:right">
                    <div style="font-size:13px;font-weight:600;color:${freqColor}">${freqDisplay}</div>
