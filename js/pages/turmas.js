...
import { auth, turmas, cursos, unidades, turnos, alunos } from '../store.js';
...
export function render(outlet) {
    ...
    let viewMode = 'minhas'; // 'minhas' | 'todas'
    const me = auth.currentUser();
    const isProfessor = me && me.role === 'professor';

    // Create a reactive proxy store to filter Data for professors
    const turmasProxy = {
        ...turmas,
        getAll: () => {
            const all = turmas.getAll();
            if (isProfessor && viewMode === 'minhas') {
                const meusCursos = me.cursos || [];
                return all.filter(t => meusCursos.includes(t.cursoId));
            }
            return all;
        }
    };
    ...
    if (isProfessor) {
        ...
        if (headerActions) {
            const tabsHtml = `
                <div class="tabs tabs-boxed" style="background:var(--surface-3); padding:4px; margin-right:8px; display:inline-flex;">
                    <a class="tab tab-active" id="tab-minhas" style="border-radius:6px; font-weight:600; padding:0 16px; height:36px; display:flex; align-items:center; cursor:pointer; color:var(--primary-500)">Minhas Turmas</a>
