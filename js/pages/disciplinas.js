...
import { disciplinas, cursos, auth } from '../store.js';
...
export function render(outlet) {
    ...
    const filteredStore = {
        ...disciplinas,
        getAll: () => {
            const all = disciplinas.getAll();
            if (isProfessor) {
                const minhasDiscs = me.disciplinas || [];
                return all.filter(d => minhasDiscs.includes(d.id));
            }
            return all;
        }
    };
    ...
    buildCrudPage(outlet, {
        ...
        filterSelects: [
            ...
            },
            {
                ...
                options: [
                    { value: 'simples', label: 'Média Simples' },
                    { value: 'ponderada', label: 'Média Ponderada' },
                    { value: 'somaPontos', label: 'Soma de Pontos' }
                ],
            },
            {
                name: 'permiteSeminarios',
                label: 'Permite Seminários',
                options: [
                    { value: 'sim', label: 'Sim' },
                    { value: 'nao', label: 'Não' }
                ],
