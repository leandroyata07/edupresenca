...
function isTyping() {
    ...
    // Recursively find the actual active element inside Shadow DOMs
...
function getActiveModal() {
    ...
    for (const m of modals) {
        const host = m.shadowRoot;
        if (host) {
            const backdrop = host.querySelector('.backdrop') || host.querySelector('#backdrop');
            if (backdrop && getComputedStyle(backdrop).display !== 'none' && m.classList.contains('open')) {
                return m;
            }
        }
    }
...
export function initShortcuts() {
    if (window._shortcutsInitialized) return;
    window._shortcutsInitialized = true;
