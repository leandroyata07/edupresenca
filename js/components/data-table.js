...
class DataTable extends HTMLElement {
  ...
  _renderShell() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .search-input:focus { border-color: var(--primary-500, #3b82f6); box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.12); }

        .toolbar-right { display: flex; align-items: center; gap: 8px; }
        .count { font-size: 12px; color: var(--text-secondary, #475569); font-weight: 500; }

        .table-wrap { overflow-x: auto; }

        table { width: 100%; border-collapse: collapse; font-size: 13px; }
        thead { background: var(--bg-surface-2, #f1f5f9); }
        th {
          padding: 10px 14px; text-align: left; font-weight: 600;
          font-size: 11px; letter-spacing: 0.05em; text-transform: uppercase;
          color: var(--text-secondary, #475569); white-space: nowrap;
          border-bottom: 1px solid var(--border-color, rgba(0,0,0,0.06));
          user-select: none;
        }
        th.sortable { cursor: pointer; }
        th.sortable:hover { color: var(--text-primary, #0f172a); }
        th.sort-asc::after  { content: ' ↑'; color: var(--primary-500, #3b82f6); }
        th.sort-desc::after { content: ' ↓'; color: var(--primary-500, #3b82f6); }
        ...
        tbody tr:hover { background: var(--bg-hover, rgba(0,0,0,0.02)); cursor: pointer; }

        td { padding: 12px 14px; color: var(--text-primary, #0f172a); vertical-align: middle; }
        ...
        .pagination {
          display: flex; align-items: center; justify-content: space-between;
          padding: 12px 16px; border-top: 1px solid var(--border-color, rgba(0,0,0,0.06));
          gap: 12px; flex-wrap: wrap;
        }
        .page-info { font-size: 12px; color: var(--text-secondary, #475569); }
        ...
        .page-btn:hover { background: var(--bg-hover, rgba(0,0,0,0.04)); color: var(--text-primary, #0f172a); }
        .page-btn.active { background: var(--primary-600, #2563eb); color: white; }
        ...
        @media (max-width: 640px) {
          ...
          tbody tr:last-child { border-bottom: 1px solid var(--border-color, rgba(0,0,0,0.06)); }
    ...
    this.shadowRoot.getElementById('tbody').addEventListener('click', (e) => {
      ...
      if (!tr) return;
      const idx = [...this.shadowRoot.getElementById('tbody').querySelectorAll('tr')].indexOf(tr);
      // Note: page is not available here, but we can find the data from the dataset or index
      if (idx >= 0 && this._filtered) {
        const start = (this._page - 1) * this._perPage;
        const rowData = this._filtered[start + idx];
        if (rowData) {
