...
// =========================================================
import { alunos, turmas, cursos, unidades, presencas, notas, disciplinas, config, auth } from '../store.js';
import { escapeHtml, getInitials, stringToHue, formatDate, calcAverage, gradeSituation, gradeSituationLabel } from '../utils.js';

const PERIODOS = ['1º Bimestre', '2º Bimestre', '3º Bimestre', '4º Bimestre', '1º Semestre', '2º Semestre'];
...
export function openBoletim(alunoId, modalEl) {
    ...
    // ── Frequência ──────────────────────────────────────
    const freqRegistros = presencas.getByAluno(aluno.id, minhasDisciplinas);
    ...
    let freqTurmaRegs = turma ? presencas.getAll().filter(p => p.alunoId === aluno.id && p.turmaId === turma.id) : [];
    if (minhasDisciplinas) {
        freqTurmaRegs = freqTurmaRegs.filter(p => minhasDisciplinas.includes(p.disciplinaId));
    }
    ...
    const discLinhas = discNomes.map(disc => {
        ...
        periodoNotas.forEach((n, idx) => {
            if (n !== null) {
                const peso = pesos[idx] !== undefined ? pesos[idx] : 1;
                wSum += n * peso;
                wTotal += peso;
            }
        });
            ...
            <table class="bol-table">
              <thead>
                <tr>
                  ...
                  ${PERIODOS.map(p => `<th class="bol-th">${p.replace('º Bimestre', 'º Bim').replace('º Semestre', 'º Sem')}</th>`).join('')}
    ...
    <table>
      <thead>
        <tr>
          ...
          ${PERIODOS.map(p => `<th>${p.replace('º Bimestre', 'º Bim').replace('º Semestre', 'º Sem')}</th>`).join('')}
          <th>Média</th>
          <th>Situação</th>
        </tr>
      </thead>
