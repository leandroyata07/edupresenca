# EduPresença 1.0.2 — Manual de Funcionalidades

> **Versão:** 1.0.2 &nbsp;|&nbsp; **Data:** Maio de 2026  
...
| 20 | Quadro de Horários | [→](#20-quadro-de-horários) |
...
## 20. Quadro de Horários

**Arquivo:** `js/pages/horarios.js` · `js/store.js` · `js/components/sidebar.js`

### O que faz
Oferece uma ferramenta completa para planejar e visualizar a grade horária semanal de aulas.
- **Grade Interativa (Gestores/Admins):** Permite agendar aulas para cada dia da semana (Segunda a Sábado) e período (1ª a 6ª aula) clicando diretamente nas células.
- **Validação de Conflitos:** O sistema impede salvar horários que gerem conflito (um mesmo professor dando duas aulas simultâneas em turmas diferentes, ou duas turmas ocupando a mesma sala de aula ao mesmo tempo).
- **Visão do Professor ("Meus Horários"):** Filtra automaticamente os horários do professor conectado, destacando a sala de aula (🚪) e a turma/curso para onde o docente deve se dirigir a cada período.
- **Impressão Direta:** Permite gerar uma visualização em formato de grade limpa pronta para impressão física ou exportação em PDF.

### Como usar
1. Acesse **Quadro de Horários** na barra lateral.
2. Na aba **Grade por Turma**, escolha a turma desejada para ver a grade dela.
3. Se for administrador/gestor, clique em qualquer espaço vago para agendar uma aula selecionando a disciplina, o professor, a sala e o horário, ou clique em uma aula agendada para editar ou remover.
4. Na aba **Grade por Professor**, visualize o quadro de aulas de qualquer docente (gestores) ou a sua própria grade (professores).
...
*Manual gerado automaticamente — EduPresença v1.0.2*
